...
| `GET` | `/api/yiyan` | 获取每日一言，后端会将外部 HTML 响应转换为纯文本 |
...
`task_ids` 用于番茄钟任务绑定，可传入多个任务 ID。为兼容旧数据，后端仍会识别单个 `task_id` 字段。
